...
Project: **Tabs to EPUB & Kindle** — Chrome MV3 extension that turns one or more open browser tabs into a Kindle-friendly EPUB. Multiple tabs become chapters with a table of contents. Optionally emails the EPUB to a Kindle via Gmail API.
...
- **`background.ts`** — MV3 service worker. Orchestrates extraction, image embedding, EPUB building, downloading, and emailing. Registers context menus. Handles all `TEST_*` and `UI_*` messages from the test harness and popup.
...
- **`email-gmail.ts`** — Gmail API client. Builds MIME messages, handles base64url encoding, auth token retry on 401/403. Enforces a 35 MB Gmail API size limit before sending.
- **`kindle-email.ts`** — Kindle-specific layer over Gmail. Email validation, `chrome.identity` token management, calls `sendGmailMessage`.
...
| `UI_BUILD_EPUB` | Build EPUB, return base64 + filename (popup writes via File System Access) |
| `UI_SAVE_TAB_IDS` | Build EPUB and download via `chrome.downloads` fallback |
...
4. **Image embedding** (`background.ts`): `collectImages` tokenizes image `src` URLs; background fetches images and replaces tokens with embedded assets.
...
- Builds a MIME multipart message with base64-encoded EPUB attachment.
...
- **Unit tests** (`npm test`): `tests/*.test.ts` — EPUB structure, ZIP format, email MIME building, directory handle IndexedDB, Gmail size limit.
...
- **E2E** (`npm run test:e2e`): Playwright with Chromium. Two spec files:
  ...
  - `ui-handlers.spec.ts` — all `UI_*` message handlers (settings, kindle email, directory, build EPUB).
